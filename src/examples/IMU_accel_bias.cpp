...
#include "../include/GNCImplementations/ImuNavigation.h"
...
int main() {
    ...
    s0.pos_inertial = Vec3{0.0, 0.0, 10.0};
    ...
    auto gravity = std::make_shared<ZeroGravityModel>();
    ...
    const Vec3 accel_bias = Vec3{0.0, 0.0, 1.0};
    const Vec3 gyro_bias = Vec3{0.0, 0.0, 1.0};

    auto sensors = std::make_unique<BiasedImuModel>(accel_bias, gyro_bias);

    auto guidance = std::make_unique<ZeroGuidance>(0.0);
    auto navigation = std::make_unique<ImuNavigation>(gravity);
    ...
    try {
        State final_State = sim.run(s0, "/Users/ashtonlowenstein/CLionProjects/Missile6DOF/IMU_accel_bias.csv");
